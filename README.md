...
每个落地代理都通过客户端现有的 `🚀 节点选择` 建立连接，因此第一跳继续由客户端自动测速或用户手动选择。Clash/Mihomo 订阅使用仅监听 `127.0.0.1` 的本地 `tunnels` 承载这条拨号链，使策略组的手动延迟测试与实际业务连接走同一路径；Sing-box 继续使用 `detour`。Worker 会让命中的分流落地覆盖当前节点路径中的全局代理，避免形成“全局代理 → 分流代理”的三跳链路。原有 **Cloudflare CDN 访问设置**、单一 `PROXYIP`、路径参数和其他代理功能保持不变。
