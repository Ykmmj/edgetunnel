...
async function forwardataTCP(host, portNum, rawData, ws, respHeader, remoteConnWrapper, yourUUID, request = null, 反代上下文 = {}, 允许木马反代 = false, 木马反代首包数据 = null) {
	...
	async function 打开TCP连接(address, port) {
		const 标准地址 = String(address || '').replace(/^\[|\]$/g, '');
		const remoteSock = TCP连接({ hostname: 标准地址, port });
		try {
...
const PROXYIPPOOL客户端业务域名 = Object.freeze({
...
function 规范化PROXYIP地址(原始地址) {
	...
	const 冒号数量 = (地址.match(/:/g) || []).length;
	if (冒号数量 > 1 && !地址.startsWith('[')) 地址 = `[${地址}]`;
	return 地址;
}

function 规范化PROXYIPPOOL地址(原始地址, 协议 = 'proxyip') {
...
function 规范化PROXYIPPOOL配置(原始配置 = {}, 旧PROXYIP = 'auto') {
	...
	const 已使用ID = new Set(), 已使用名称 = new Set();
	const 节点 = [];
	...
	const 添加节点 = (节点配置, 索引) => {
		...
		已使用ID.add(id);
		let 名称 = String(节点配置.名称 || `PROXYIP ${索引 + 1}`).replace(/[\r\n\0]/g, ' ').trim().slice(0, 64) || `PROXYIP ${索引 + 1}`;
		const 基础名称 = 名称;
		let 名称后缀 = 2;
		while (已使用名称.has(名称)) 名称 = `${基础名称} (${名称后缀++})`.slice(0, 64);
		已使用名称.add(名称);
		节点.push({ id, 名称, 协议, 地址, 启用: 节点配置.启用 !== false });
	};

	原始节点.slice(0, 32).forEach(添加节点);
...
async function 检测PROXYIP可用性(request, PROXYIP地址, uuid) {
	...
	try {
		...
		for (const [hostname, port] of 候选列表) {
			let tcpSocket = null, tlsSocket = null;
			try {
				const 拨号主机 = String(hostname || '').replace(/^\[|\]$/g, '');
				tcpSocket = TCP连接({ hostname: 拨号主机, port });
				await Promise.race([
					tcpSocket.opened,
...
function 应用ClashPROXYIPPOOL补丁(原始YAML, config_JSON = {}) {
	...
	const 解析YAML标量 = raw => {
		...
		if (value.startsWith('"')) {
			try { return JSON.parse(value.match(/^"(?:\\.|[^"])*"/)?.[0] || value) } catch (_) { }
		}
		if (value.startsWith("'")) {
			const match = value.match(/^'(?:''|[^'])*'/)?.[0];
			if (match) return match.slice(1, -1).replace(/''/g, "'");
		}
		return value.split(/\s+#/)[0].trim();
	};
	const 获取行内名称 = line => {
		const block = line.match(/^\s*-\s*name:\s*(.+)$/);
		if (block) return 解析YAML标量(block[1]);
...
function 应用SingboxPROXYIPPOOL补丁(config, config_JSON = {}) {
	...
	const 新规则 = Object.entries(PROXYIPPOOL客户端业务域名).map(([组名, 域名列表]) => ({ domain_suffix: 域名列表, action: 'route', outbound: 组名 }));
	config.route.rules = [...新规则, ...原规则];
	return config;
}
...
function Clash订阅配置文件热补丁(Clash_原始订阅内容, config_JSON = {}) {
	...
	let clash_yaml = 应用ClashPROXYIPPOOL补丁(Clash_原始订阅内容.replace(/mode:\s*Rule\b/g, 'mode: rule'), config_JSON);
...
async function Singbox订阅配置文件热补丁(SingBox_原始订阅内容, config_JSON = {}) {
	...
	try {
		...
		应用SingboxPROXYIPPOOL补丁(config, config_JSON);
