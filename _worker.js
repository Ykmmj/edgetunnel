const Version = '2026-07-22 12:00:00';
let config_JSON, 缓存SOCKS5白名单 = null, 调试日志打印 = false;
let PROXYIPPOOL运行缓存 = { 过期时间: 0, 配置: null };
...
export default {
	async fetch(request, env, ctx) {
		...
		const Pages静态页面 = String(env.PAGES_URL || 默认Pages静态页面).trim().replace(/\/+$/, '') || 默认Pages静态页面;
		...
		if (访问路径 === 'version') {// 版本信息接口
			...
			反代上下文.PROXYIPPOOL = await 读取PROXYIPPOOL运行配置(env);
				...
				if (区分大小写访问路径 === 加密秘钥 && 加密秘钥 !== '勿动此默认密钥，有需求请自行通过添加变量KEY进行修改') {//快速订阅
					...
					if (访问路径 === 'admin/log.json') {// 读取日志内容
						...
						if (url.searchParams.has('proxyip')) {
							const 检测结果 = await 检测PROXYIP可用性(request, url.searchParams.get('proxyip'), userID);
							return new Response(JSON.stringify(检测结果, null, 2), { status: 200, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
						}
					...
					if (访问路径 === 'admin/init') {// 重置配置为默认值
						...
						if (访问路径 === 'admin/config.json') { // 保存config.json配置
							try {
								...
								// 保存到 KV
								if (!newConfig.反代 || typeof newConfig.反代 !== 'object') newConfig.反代 = {};
								newConfig.反代.PROXYIPPOOL = 规范化PROXYIPPOOL配置(newConfig.反代.PROXYIPPOOL, newConfig.反代[特征码字典[0]]);
								await env.KV.put('config.json', JSON.stringify(newConfig, null, 2));
								config_JSON = newConfig;
								PROXYIPPOOL运行缓存 = {
									过期时间: Date.now() + 5000,
									配置: newConfig.反代.PROXYIPPOOL.启用 ? newConfig.反代.PROXYIPPOOL : null
								};
...
async function forwardataTCP(host, portNum, rawData, ws, respHeader, remoteConnWrapper, yourUUID, request = null, 反代上下文 = {}, 允许木马反代 = false, 木马反代首包数据 = null) {
	...
	let ctx反代兜底 = 反代上下文.反代兜底 !== undefined ? 反代上下文.反代兜底 : true;
	let 反代数组索引 = 0;
	const 连接超时毫秒 = 1000;
	let 已通过代理发送首包 = false;
	const TCP连接 = 创建请求TCP连接器(request);
	...
	const PROXYIPPOOL出口 = !ctx代理类型 && !使用木马反代 ? 选择PROXYIPPOOL出口(反代上下文.PROXYIPPOOL, host) : null;
	if (PROXYIPPOOL出口) {
		ctx反代IP = PROXYIPPOOL出口.节点.地址;
		ctx反代兜底 = false;
	}
	log(`[TCP转发] 目标: ${host}:${portNum} | 反代IP: ${ctx反代IP} | 反代兜底: ${ctx反代兜底 ? '是' : '否'} | 反代类型: ${ctx代理类型 || 'proxyip'} | 全局: ${ctx代理全局 ? '是' : '否'}${PROXYIPPOOL出口 ? ` | PROXYIPPOOL: ${PROXYIPPOOL出口.业务名称} -> ${PROXYIPPOOL出口.节点.名称}` : ''}`);
	...
	if (PROXYIPPOOL出口) {
		log(`[PROXYIPPOOL] ${PROXYIPPOOL出口.业务名称} 流量使用 ${PROXYIPPOOL出口.节点.名称}: ${PROXYIPPOOL出口.节点.地址}`);
		try {
			await connecttoPry();
		} catch (err) {
			log(`[PROXYIPPOOL] ${PROXYIPPOOL出口.节点.名称} 连接失败: ${err.message}`);
			throw err;
		}
...
const PROXYIPPOOL业务域名 = Object.freeze({
	ChatGPT: ['chatgpt.com', 'openai.com', 'oaistatic.com', 'oaiusercontent.com', 'sora.com'],
...
function 清空PROXYIPPOOL运行缓存() {
	PROXYIPPOOL运行缓存 = { 过期时间: 0, 配置: null };
}

function 规范化PROXYIP地址(原始地址) {
	let 地址 = String(原始地址 || '').replace(/[\r\n\0]/g, '').trim().slice(0, 512);
	地址 = 地址.replace(/^\/?(?:proxyip:\/\/|proxyip=)/i, '').split('#')[0].trim().replace(/\s+/g, '');
	const CIDR匹配 = 地址.match(/\/(\d{1,3})$/);
	if (CIDR匹配) {
		const 主体 = 地址.slice(0, -CIDR匹配[0].length);
		const 前缀长度 = Number(CIDR匹配[1]);
		const 是IPv4 = /^\d{1,3}(?:\.\d{1,3}){3}$/.test(主体);
		const 是IPv6 = 主体.includes(':');
		if ((是IPv4 && 前缀长度 <= 32) || (是IPv6 && 前缀长度 <= 128)) 地址 = 主体;
	}
	return 地址;
}

function 规范化PROXYIPPOOL配置(原始配置 = {}, 旧PROXYIP = 'auto') {
	const 配置 = 原始配置 && typeof 原始配置 === 'object' && !Array.isArray(原始配置) ? 原始配置 : {};
	const 原始节点 = Array.isArray(配置.节点) ? 配置.节点 : [];
	const 已使用ID = new Set();
	const 节点 = [];

	const 添加节点 = (节点配置, 索引) => {
		if (!节点配置 || typeof 节点配置 !== 'object' || Array.isArray(节点配置)) return;
		const 地址 = 规范化PROXYIP地址(节点配置.地址);
		if (!地址 || 地址.toLowerCase() === 'auto') return;
		let id = String(节点配置.id || '').replace(/[^a-zA-Z0-9_-]/g, '').slice(0, 64) || `proxyip-${索引 + 1}`;
		const 基础ID = id;
		let 后缀 = 2;
		while (已使用ID.has(id)) id = `${基础ID}-${后缀++}`;
		已使用ID.add(id);
		const 名称 = String(节点配置.名称 || `PROXYIP ${索引 + 1}`).replace(/[\r\n\0]/g, ' ').trim().slice(0, 64) || `PROXYIP ${索引 + 1}`;
		节点.push({ id, 名称, 地址, 启用: 节点配置.启用 !== false });
	};

	原始节点.forEach(添加节点);
	const 旧地址 = 规范化PROXYIP地址(旧PROXYIP);
	if (节点.length === 0 && 旧地址 && 旧地址.toLowerCase() !== 'auto') {
		添加节点({ id: 'legacy-default', 名称: '原 PROXYIP', 地址: 旧地址, 启用: true }, 0);
	}

	const 可用节点 = 节点.filter(节点配置 => 节点配置.启用);
	const 可用ID = new Set(可用节点.map(节点配置 => 节点配置.id));
	const 默认出口候选 = String(配置.默认出口 || '').trim();
	const 默认出口 = 可用ID.has(默认出口候选) ? 默认出口候选 : (可用节点[0]?.id || null);
	const 原始业务分流 = 配置.业务分流 && typeof 配置.业务分流 === 'object' ? 配置.业务分流 : {};
	const 规范化业务出口 = value => {
		const id = String(value || '').trim();
		return id && 可用ID.has(id) ? id : null;
	};

	return {
		启用: Boolean(配置.启用) && 可用节点.length > 0,
		节点,
		默认出口,
		业务分流: {
			ChatGPT: 规范化业务出口(原始业务分流.ChatGPT),
			Claude: 规范化业务出口(原始业务分流.Claude),
			Netflix: 规范化业务出口(原始业务分流.Netflix)
		}
	};
}

async function 读取PROXYIPPOOL运行配置(env) {
	if (!env?.KV || typeof env.KV.get !== 'function') return null;
	const 当前时间 = Date.now();
	if (PROXYIPPOOL运行缓存.过期时间 > 当前时间) return PROXYIPPOOL运行缓存.配置;
	const 上次有效配置 = PROXYIPPOOL运行缓存.配置;

	try {
		const 配置文本 = await env.KV.get('config.json');
		if (!配置文本) {
			PROXYIPPOOL运行缓存 = { 过期时间: 当前时间 + 5000, 配置: null };
			return null;
		}
		const 完整配置 = JSON.parse(配置文本);
		const 反代配置 = 完整配置?.反代 && typeof 完整配置.反代 === 'object' ? 完整配置.反代 : {};
		const 池配置 = 规范化PROXYIPPOOL配置(反代配置.PROXYIPPOOL, 反代配置[特征码字典[0]]);
		const 运行配置 = 池配置.启用 ? 池配置 : null;
		PROXYIPPOOL运行缓存 = { 过期时间: 当前时间 + 5000, 配置: 运行配置 };
		return 运行配置;
	} catch (error) {
		console.error(`[PROXYIPPOOL] 读取运行配置失败: ${error.message || error}`);
		PROXYIPPOOL运行缓存 = { 过期时间: 当前时间 + 2000, 配置: 上次有效配置 || null };
		return 上次有效配置 || null;
	}
}

function 匹配PROXYIPPOOL业务(目标域名) {
	const hostname = String(目标域名 || '').trim().toLowerCase().replace(/\.$/, '');
	if (!hostname) return null;
	for (const [业务名称, 域名列表] of Object.entries(PROXYIPPOOL业务域名)) {
		if (域名列表.some(域名 => hostname === 域名 || hostname.endsWith(`.${域名}`))) return 业务名称;
	}
	return null;
}

function 选择PROXYIPPOOL出口(池配置, 目标域名) {
	if (!池配置?.启用 || !Array.isArray(池配置.节点)) return null;
	const 业务名称 = 匹配PROXYIPPOOL业务(目标域名);
	const 业务出口ID = 业务名称 ? 池配置.业务分流?.[业务名称] : null;
	const 出口ID = 业务出口ID || 池配置.默认出口;
	const 节点 = 池配置.节点.find(节点配置 => 节点配置.启用 && 节点配置.id === 出口ID);
	return 节点 ? { 节点, 业务名称: 业务名称 || '默认' } : null;
}

async function 检测PROXYIP可用性(request, PROXYIP地址, uuid) {
	const startTime = Date.now();
	const 原始地址 = 规范化PROXYIP地址(PROXYIP地址);
	if (!原始地址) return { success: false, error: 'PROXYIP 地址不能为空', proxy: 'proxyip://', responseTime: 0 };

	let 最后错误 = null;
	try {
		const 检测主机 = 'cloudflare.com';
		const 候选列表 = await 解析地址端口(原始地址, 检测主机, uuid);
		if (!候选列表.length) throw new Error('未解析到可用的 PROXYIP 地址');
		const TCP连接 = 创建请求TCP连接器(request);
		const encoder = new TextEncoder(), decoder = new TextDecoder();

		for (const [hostname, port] of 候选列表) {
			let tcpSocket = null, tlsSocket = null;
			try {
				...
				while (responseBuffer.length < 64 * 1024) {
					const value = await Promise.race([
						tlsSocket.read(),
						new Promise((_, reject) => setTimeout(() => reject(new Error('读取响应超时')), 5000))
					]);
					if (!value) break;
					if (value.byteLength === 0) continue;
					responseBuffer = 拼接字节数据(responseBuffer, value);
				...
				if (!Number.isFinite(statusCode) || statusCode < 200 || statusCode >= 300) throw new Error(`PROXYIP 检测请求失败: ${statusLine || '无效响应'}`);
				const ip = response.match(/(?:^|\n)ip=(.*)/)?.[1]?.trim();
				const loc = response.match(/(?:^|\n)loc=(.*)/)?.[1]?.trim();
				if (!ip || !loc) throw new Error('PROXYIP 检测响应无效');
				return { success: true, proxy: `proxyip://${原始地址}`, candidate: `${hostname}:${port}`, ip, loc, responseTime: Date.now() - startTime };
			} catch (error) {
				最后错误 = error;
			} finally {
				try { tlsSocket ? tlsSocket.close() : await tcpSocket?.close?.() } catch (e) { }
	...
	return { success: false, error: 最后错误?.message || 'PROXYIP 检测失败', proxy: `proxyip://${原始地址}`, responseTime: Date.now() - startTime };
}
...
async function 读取config_JSON(env, hostname, userID, UA = "Mozilla/5.0", 重置配置 = false) {
	...
	config_JSON.反代.PROXYIPPOOL = 规范化PROXYIPPOOL配置(config_JSON.反代.PROXYIPPOOL, config_JSON.反代[_p]);
